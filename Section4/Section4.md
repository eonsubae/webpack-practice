...
플러그인의 필요성
* Loader를 사용하면 자바스크립트가 아닌 파일도 자바스크립트에서 사용할 수 있다
* 그러나 기존에 없었던 혹은 잘 사용되지 않는 파일이라면 로더가 없을 수도 있다
...
h1 {
  color: gray; }
```
* hello-world-button 컴포넌트의 scss코드와 heading 컴포넌트의 scss코드가 합쳐진 것을 확인할 수 있다
...
* 그렇다면 이 문제를 어떻게 해결할 수 있을까
  - 만일 사이트 공급자가 새로운 기능을 추가했다면 사용자 컴퓨터는 이를 인식할 수 있어야한다
  - 이 문제는 새로운 기능이 추가된 파일을 만들 때에는 파일이름을 변경하는 것으로 해결할 수 있다
...
* 자바스크립트에 추가했던 const ten = 10;을 삭제했기 때문에 자바스크립트 파일도 새로운 해쉬값이 적용된 파일이 생성됐다.
...
```
* 앞서 빌드한 파일을 Copy and Paste 한다
* 그런 다음 handlebars 문법을 사용해서 동적으로 title과 description을 입력받도록 수정했다
